class Scoresheet {
	...
	saveScore() {
		if (this.controller.saveScore) {
			...
			// Dan mode: use DanExam result for pass determination
			var clearReached, crown = ""
			if (this.controller.isDanMode && this.controller.danExam) {
				var danResult = this.controller.danExam.getResult()
				clearReached = danResult === 'gold' || danResult === 'pass'
				if (danResult === 'gold') {
					crown = 'gold'
				...
				// Normal mode
				clearReached = this.controller.game.rules.clearReached(this.resultsObj.gauge)
				if (clearReached) {

class ParseTja {
	...
	init(file, difficulty, stars, offset, metaOnly) {
		...
		// Dan mode tracking
		this.isDan = false
		this.danSongs = []  // [{title, wave, delay, startMs, endMs}, ...]
		this.danExams = []  // [{type, red, gold, scope}, ...]
	...
	parseMetadata() {
		...
		var currentCourse = {}
		var courseName = "oni"
		var inDanCourse = false  // Track if we're in a Dan course
		for (var lineNum = 0; lineNum < this.data.length; lineNum++) {
			...
			if (line.slice(0, 1) === "#") {
				...
				if (line.indexOf(":") > 0) {
					...
					// Parse EXAM1-4 for Dan mode
					else if (inDanCourse && name.match(/^exam[1-4]$/)) {
						var examParts = value.split(",")
						if (examParts.length >= 4) {
							var exam = {
								id: parseInt(name.slice(-1)),
								type: examParts[0].trim().toLowerCase(),
								red: parseFloat(examParts[1].trim()),
								gold: parseFloat(examParts[2].trim()),
								scope: examParts[3].trim().toLowerCase()
							}
							// Only add EXAM1-3 to global exams; EXAM4 is per-song
							if (exam.id <= 3) {
								this.danExams.push(exam)
							}
						}
					}
			...
			if (line.slice(0, 1) === "#") {
				...
				switch (name) {
					...
					case "lyric":
						...
						break
					case "nextsong":
						// Dan mode: next song marker
						if (this.isDan) {
							// Close previous song timing
							if (this.danSongs.length > 0) {
								this.danSongs[this.danSongs.length - 1].endMs = ms
							}
							// Parse: title,subtitle,genre,wave,scoreInit,scoreDiff
							var parts = value.split(",")
							var songInfo = {
								title: parts[0] ? parts[0].trim() : "",
								subtitle: parts[1] ? parts[1].trim() : "",
								genre: parts[2] ? parts[2].trim() : "",
								wave: parts[3] ? parts[3].trim() : "",
								scoreInit: parts[4] ? parseFloat(parts[4]) : 0,
								scoreDiff: parts[5] ? parseFloat(parts[5]) : 0,
								startMs: ms,
								endMs: null,
								songIndex: this.danSongs.length + 1
							}
							this.danSongs.push(songInfo)
							// Insert song break marker
							insertNote({
								type: "songBreak",
								songInfo: songInfo,
								bpm: bpm,

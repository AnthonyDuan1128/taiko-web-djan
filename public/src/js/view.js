class View {
	...
	init(controller) {
		...
		if (this.multiplayer !== 2) {
			...
			if (this.controller.touchEnabled) {
				...
				if (this.multiplayer || this.controller.isDanMode) {
	...
	refresh() {
		...
		if (this.portrait) {
			...
			// Dan exam conditions panel (portrait)
			if (this.controller.isDanMode) {
				this.drawDanExamPanel(ctx, 10, 580, 0.85)
			}
			...
			// Dan exam conditions panel (landscape)
			if (this.controller.isDanMode) {
				this.drawDanExamPanel(ctx, 10, 400, 1)
			}
	...
	/**
	 * Draw Dan exam conditions panel during gameplay
	 */
	drawDanExamPanel(ctx, x, y, scale) {
		if (!this.controller.isDanMode || !this.controller.danExam) {
			return
		}

		var danExam = this.controller.danExam
		var exams = danExam.exams
		...
		// Title
		ctx.font = this.draw.bold(this.font) + "14px " + this.font
		ctx.fillStyle = "#ffd700"
		ctx.textAlign = "center"
		ctx.textBaseline = "top"
		ctx.fillText("段位道場", panelW / 2, 5)
		...
		for (var i = 0; i < exams.length; i++) {
			var exam = exams[i]
			var result = danExam.checkExam(exam)
			var yPos = 28 + i * 32

			// Condition type name
			var typeName = DanExam.getExamTypeName(exam.type)
			ctx.fillStyle = "#fff"
			ctx.fillText(typeName, 8, yPos)

			// Current value / target
			var currentText = Math.round(result.current)
			if (exam.type === 'g') {
				currentText = result.current.toFixed(1) + "%"
			}

			// Color based on status
			if (result.status === 'gold') {
				ctx.fillStyle = "#ffd700"  // Gold
			} else if (result.status === 'red') {
				ctx.fillStyle = "#ff6b6b"  // Red pass
			} else {
				ctx.fillStyle = "#888"     // Fail
			}

			ctx.textAlign = "right"
			ctx.fillText(currentText, panelW - 60, yPos)

			// Target values
			ctx.fillStyle = "#aaa"
			ctx.font = "10px " + this.font
			var targetText = result.isLowerBetter
				? "≤" + exam.red + "/" + exam.gold
				: "≥" + exam.red + "/" + exam.gold
			ctx.fillText(targetText, panelW - 8, yPos + 12)
			ctx.font = "12px " + this.font
			ctx.textAlign = "left"
		}

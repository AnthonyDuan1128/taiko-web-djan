class Controller {
	...
	init(selectedSong, songData, autoPlayEnabled, multiplayer, touchEnabled) {
		...
		// Dan mode initialization
		this.isDanMode = this.parsedSongData.isDan || (selectedSong.is_dan === true)
		if (this.isDanMode) {
			// Create DanExam tracker with parsed exam data
			var danExams = selectedSong.dan_exams || this.parsedSongData.danExams || []
			var danSongs = selectedSong.dan_songs || this.parsedSongData.danSongs || []
			this.danExam = new DanExam(danExams, danSongs)
			this.saveScore = false  // Don't save individual scores for Dan mode
			console.log("Dan mode initialized:", danExams.length, "exams,", danSongs.length, "songs")
		}
	}
	run(syncWith) {
		if (syncWith) {
	...
	gameEnded() {
		...
		// Dan mode: use DanExam result
		if (this.isDanMode && this.danExam) {
			var danResult = this.danExam.getResult()
			if (danResult === 'gold') {
				vp = "fullcombo"  // Use fullcombo sound for gold pass
				this.playSound("v_fullcombo", 1.350)
			} else if (danResult === 'pass') {
				vp = "clear"
			} else {
				vp = "fail"
			}
	...
	togglePause(forcePause, pauseMove, noSound) {
		// Disable pause in Dan mode
		if (this.isDanMode && !forcePause) {
			return
		}
		if (this.multiplayer === 1) {

	...
	redraw() {
		...
		this.draw.songFrame({
			...
			innerContent: (x, y, w, h) => {
				...
				// Play stats display (only in difficulty selection, not song selection)
				if (!songSel) {
					...
					if (stats) {
						...
						this.draw.layeredText({
							ctx: ctx,
							text: strings.playCount + ": " + stats.playCount,
							fontSize: 20,
							fontFamily: this.font,
							align: "left",
							x: _x - 18,
						...
						// Weekly high score
						var weeklyText = stats.weeklyHighScore !== null
							? strings.weeklyHighScore + ": " + stats.weeklyHighScore
							: strings.weeklyHighScore + ": --"
						this.draw.layeredText({
							ctx: ctx,
							text: weeklyText,
							fontSize: 20,

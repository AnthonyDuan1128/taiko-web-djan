class Game {
	...
	checkBalloon(circle) {
		...
		// Dan exam drumroll tracking
		if (this.controller.isDanMode && this.controller.danExam) {
			this.controller.danExam.recordHit('drumroll', 1)
		}
	...
	checkDrumroll(circle, keysKa) {
		...
		// Dan exam drumroll tracking
		if (this.controller.isDanMode && this.controller.danExam) {
			this.controller.danExam.recordHit('drumroll', dai ? 2 : 1)
		}
		this.view.setDarkBg(false)
	}
	...
	updateGlobalScore(score, multiplier, gogoTime) {
		...
		switch (score) {
			case 450:
				...
				// Dan exam tracking
				if (this.controller.isDanMode && this.controller.danExam) {
					this.controller.danExam.recordHit('good', 1)
				}
				break
			case 230:
				this.globalScore.ok++
				...
				// Dan exam tracking
				if (this.controller.isDanMode && this.controller.danExam) {
					this.controller.danExam.recordHit('ok', 1)
				}
				break
			case 0:
				...
				// Dan exam tracking
				if (this.controller.isDanMode && this.controller.danExam) {
					this.controller.danExam.recordHit('bad', 1)
				}
				break
		}
